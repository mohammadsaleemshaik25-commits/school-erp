...
@section('content')
    <div class="bg-white rounded-lg shadow-sm border p-4">
        <div class="mb-4">
            <h2 class="text-xl font-semibold text-slate-900">Fee Collection Report</h2>
            <p class="text-sm text-slate-600">Daily transaction-level collection details.</p>
        </div>
        <form method="GET" class="mb-4">
            <label class="text-sm text-gray-700">Date</label>
            <input type="date" name="date" value="{{ $selectedDate }}" class="border px-2 py-1 rounded text-sm">
        ...
        <div class="overflow-x-auto rounded border">
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="5" class="p-3 text-center text-gray-500">No fee collections found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
